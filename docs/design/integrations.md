...
> 核心文件：`src/main/managers/feishu-bridge.js` (1288行) + `feishu-event-client.js` + `feishu-message-api.js`
...
飞书使用自有交互卡片系统（未接入共享命令层）：
- 卡片构建: `_buildHelpCard` / `_buildStatusCard` / `_buildSessionsCard` / `_buildHistoryChoiceCard`
- 卡片动作: `FeishuEventClient` 处理 `card.action.trigger` → `_handleCardAction`
- 共享模块: 使用 `ImSessionMapper`、`ImReplyCollector`、`ImFrontendNotifier`

---

## 企业微信桥接

> 核心文件：`src/main/managers/enterprise-weixin-bridge.js` (1759行) + `wecom-cli-manager.js`
