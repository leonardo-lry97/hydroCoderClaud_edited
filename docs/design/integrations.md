...
- 卡片层: 同时保留飞书自有交互卡片（`buildHelpCard` / `buildStatusCard` / `buildHistoryChoiceCard` / `buildResultCard` 等已迁移至 `im-card-renderers/feishu-card-renderer.js`）
