...
- 每个 embedded app 都必须有明确的“当前会话”概念
- 当前会话变化时，必须同步调用宿主桥更新主进程侧 current session 指针
- 当前会话变化不仅包括首次创建，还包括 `reopen`、显式切换、`/clear`、点击“新建会话”
- 不允许只在 renderer 的 `localStorage` 里记住 last session，却不同步主进程运行态
...
- 如果该 app 要支持“当前会话绑定”的定时任务联动，`EmbeddedAgentPanel` 或等价宿主必须负责维护 current session 同步
...
- 上下文同步与 current session 同步是两条并行状态线，不能用其中一条替代另一条
...
- 这些偏好仅用于 embedded 会话创建/恢复时的默认模型选择，不代表定时任务会持久持有一份独立模型配置
