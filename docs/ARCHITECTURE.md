...
> v1.7.69+ | 本文档是架构设计的入口，概述整体设计和模块关系，详细实现见各子文档。
...
主窗口菜单 / Notebook 入口
  ...
  → EmbeddedAppRuntimeManager 持有当前 app context / command bridge
...
| 类别 | 文件数 | 关键模块 |
|------|--------|---------|
| 页面（12 个 BrowserWindow 页面入口） | 90+ | 10 个传统桌面页面：main + notebook + 8 个管理窗口；2 个内嵌 app 页面：embedded-app-demo, hydrology-workbench |
...
| [内嵌 App 设计与实现标准](design/embedded-app-development-standard.md) | 内嵌 app 架构分层、复用约束、水文工作台案例与验收清单 | — |
| [内嵌 App 开发 SOP](design/embedded-app-development-sop.md) | 新增一个内嵌 app 的执行流程、阶段检查点、常见失败模式与提交前检查 | — |
